return { -- Fuzzy Finder (files, lsp, etc) New text
  ...
  config = function()
    ...
    vim.keymap.set('n', '<leader>ss', builtin.git_files, { desc = '[S]earch Git [F]iles' })
    ...
    vim.keymap.set(
      ...
      { desc = '[S]earch [E]verything' }
    )
    vim.keymap.set('n', '<leader>sb', builtin.builtin, { desc = '[S]earch [S]elect Telescope' })
    vim.keymap.set({ 'n', 'v' }, '<leader>sw', builtin.grep_string, { desc = '[S]earch current [W]ord' })
    vim.keymap.set('n', '<leader>sg', builtin.live_grep, { desc = '[S]earch by [G]rep' })
    vim.keymap.set('n', '<leader>sG', function() builtin.grep_string { search = vim.fn.input 'Grep ❯ ' } end, { desc = 'grep in cmd' })
